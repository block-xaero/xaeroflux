            ...
            match found {
                ...
                Some(existing_clock) => {
                    ...
                    if max_peer_logical >= current_wall_time && (max_peer_logical - current_wall_time) <= u16::MAX as u64 {
                        let new_clock = XaeroClock::with_base(current_wall_time, (max_peer_logical - current_wall_time) as u16);
...
mod vector_clock_tests {
    ...
    fn test_vector_clock_fits_in_ring_buffer() {
        ...
        let event = EventUtils::create_pooled_event::<1024>(vc_bytes, 1).expect("Should create event successfully");
    ...
    fn test_ring_buffer_flow_with_larger_events() {
        ...
        let input_event = EventUtils::create_pooled_event::<1024>(vc_bytes, 1).expect("Should create input event");
        ...
        if let Some(event) = input_reader.next() {
            ...
            let output_event = EventUtils::create_pooled_event::<1024>(output_vc_bytes, 2).expect("Should create output event");
